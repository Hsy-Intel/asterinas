...
impl X2Apic {
    ...
    pub fn enable(&mut self) {
        ...
        unsafe {
            ...
            // TODO: Prevent Iago attack: Verify SIVR write operation succeeded in Intel TDX environment.
            // This WRMSR triggers #VE exception, delegating SIVR configuration to untrusted VMM.
            // A malicious VMM/Host could exploit this to:
            // - Intercept MSR writes via MSR bitmap and silently ignore or modify values
            // - Perform timing attacks by delaying MSR write operations
            // - Partially tamper with written values (e.g., disable APIC while keeping vector number)
            // - Use delayed tampering where initial readback succeeds but value changes later
            // - Block writes through hardware virtualization features or host kernel hooks
            // Additionally, hardware-level risks include:
            // - Platform firmware locks preventing SIVR modification
            // - Spurious vector 15 conflicts with platform-specific interrupt usage
            // - APIC enable bit ignored due to processor power states or SMM interference
            // Consider implementing: immediate readback verification, timing analysis to detect
            // VM exits, multiple consistency checks over time and cross-validation with APIC
            // functionality tests.
            let svr: u64 = (1 << 8) | 15;
...
impl super::Apic for X2Apic {
    fn id(&self) -> u32 {
        ...
        // This WRMSR triggers #VE exception, delegating APIC ID retrieval to untrusted VMM.
        // A malicious VMM/Host could exploit this to:
        // - Invalid APIC ID values outside processor's valid range or conflicting with other CPUs
        ...
        // - Reserved or out-of-spec APIC ID ranges that could cause system instability
    ...
    fn version(&self) -> u32 {
        // TODO: Prevent Iago attack: Validate APIC version for information disclosure in Intel TDX environment.
        // This RDMSR triggers #VE exception that could leak sensitive information
        // through log output or be used for system fingerprinting by malicious VMM/Host.
        // Consider basic validation: ensure version number is within reasonable range to prevent log pollution attacks.
